...
from .mnist_prep import prep_data_and_dirs, get_admission_th, prep_deep_neural_network_and_data_loader
from .dnn import device

def first_mnist_example(args, dargs,parser, TOGGLES):
    print('first_mnist_example...')
    # deprecate TOGGLES next time, kinda useless
...
def ces_(dargs, parser, BOOLS):
    from .mnist_showcase_select import ces
    ces(dargs, parser, BOOLS)

def hyper_(dargs, parser, BOOLS):
    from .mnist_hyper import mnist_hyper_
    mnist_hyper_(dargs, parser, BOOLS)

def train_(dargs, parser, BOOLS, convNN=False):
    print('train_ (training on all)')

    modelname = 'mnist_k%s.pth'%(str(dargs['kwidth']))
...
def result_(dargs):
    ...
    net.print_final_() # can be too much printed, so we suppress it
