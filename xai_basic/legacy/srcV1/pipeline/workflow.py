...
import pipeline.training.resnet34_ten_classes as tr # also check debug mode here
...
def workflow1():
    ...
    # * 4gb dedicated gpu memory, cannot handle (3,512,512) image, but can handle (3,224,224). Should be ok in
    # should be fine if you have decent GPU.
    ...
    N_REGULAR_EVALUATION_EPOCH = 4 # [PART 2]
    ALLOW_NOPTIM = 1 # allow evaluation of NON-OPTIMALLY TUNED MODEL [PART 2.2]
    config_data['model_name'] = 'workflow1_0001'
    ...
    if TOGGLE['PART1']:
        ...
        config_data['n_epoch'] = 2
        tr.training_resnet34_ten_classes(config_data=config_data, realtime_update=realtime_update)
        end = time.time()
        elapsed = end - start
    ...
    if TOGGLE['PART1.2']:   
        print('\nPART1.2')     
        ev.eval_resnet34_ten_classes_plot_loss(config_data=config_data, plot_mode='savefig')
    ...
    if TOGGLE['PART2']:
        ...
        for branch_number in BRANCHES_PER_MODEL:
            ...
            ev.eval_resnet34_ten_classes_branch_validation_info(config_data=config_data)
            print('\nPART2 branch %s DONE\n\n'%(str(branch_number)))
    ...
    """ PART 3 """
    msg = 'No model for XAI evaluation exists. Please use part2 or part2.2 to build .optim or .noptim models'
    if TOGGLE['PART3']:
        print('\nPART3')
        ...
        for branch_number in BRANCHES_PER_MODEL:
            ...
            for xai_mode in XAI_MODES: 
                ...
                ev.eval_resnet34_ten_classes_xai(config_data=config_data, FIND_OPTIM_BRANCH_MODEL=True, realtime_update=realtime_update)
                # output is .xai data
            end = time.time()
            elapsed = end - start
...
"""
The equivalent sequence up to part 3.2 is:
