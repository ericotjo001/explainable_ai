from pipeline.training.alexnet_ten_classes_utils import *
import pipeline.data.prepare_10classes_data as data10
from pipeline.workflow_config import DATA_SIZE_ALEXNET

def training_alexnet_ten_classes(config_data=None, realtime_update=True):
    if config_data is None: config_data = DEFAULT_CONFIG_DATA
    print('training_alexnet_ten_classes(). training scheme:%s'%(str(config_data['training_scheme'])))
