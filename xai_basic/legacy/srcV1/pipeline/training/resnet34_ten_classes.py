from pipeline.training.resnet34_ten_classes_utils import *
import pipeline.data.prepare_10classes_data as data10

def training_resnet34_ten_classes(config_data=None, realtime_update=True):
    if config_data is None: config_data = DEFAULT_CONFIG_DATA
    print('training_resnet34_ten_classes(). training scheme:%s'%(str(config_data['training_scheme'])))
                ...
                for i, data in enumerate(trainloader, 0):
                    ...
                    if config_data['training_scheme'] == 'regular_evaluation':
                        if evaluator.early_stopping: 
                            ...
                            return
                    DEBUG_SIGNAL = do_DEBUG_VIEW_BATCH_OF_NINE(DEBUG_VIEW_BATCH_OF_NINE, x=x, y0=y0)
                    DEBUG_SIGNAL = do_DEBUG_ONE_ITER_ONE_EPOCH(DEBUG_ONE_ITER_ONE_EPOCH, x=x, y0=y0,y=y, net=net)
                    if DEBUG_N_ITER_TWO_EPOCHS>0 and i>=DEBUG_N_ITER_TWO_EPOCHS-1: break
                    if DEBUG_SIGNAL: break
            if DEBUG_N_ITER_TWO_EPOCHS>0 and epoch>=1: break
            if DEBUG_SIGNAL: break
