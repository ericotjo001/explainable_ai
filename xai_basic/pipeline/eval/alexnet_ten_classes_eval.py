from pipeline.training.alexnet_ten_classes import *
...
def eval_alexnet_ten_classes_xai(config_data=None, singleton_scope=False, FIND_OPTIM_BRANCH_MODEL=False, realtime_update=False, ALLOW_ADHOC_NOPTIM=False):
    # print('eval_alexnet_ten_classes_xai()')
...
def eval_alexnet_ten_classes_plot_loss(config_data=None, **kwargs):
    print('eval_alexnet_ten_classes_plot_loss()')
    if config_data is None: config_data = DEFAULT_CONFIG_DATA

    from pipeline.eval.evaluation_utils import eval_plot_loss
    eval_plot_loss(initiate_or_load_model, config_data, **kwargs)

def eval_alexnet_ten_classes_branch_validation_info(config_data=None):
