...
import pipeline.training.vgg_ten_classes as tr
...
def workflow3():
    ...
    # Testing with vgg
    XAI_MODES = [ # likely to raise many memory issues if your ram is small
        'Saliency', 
        # 'IntegratedGradients', 
        'InputXGradient', 
        # 'DeepLift', 
    ...
    if FULL_DATA_MODE:
        ...
        pass # use default, small_training_data_10c etc, see pipeline.prepare_10classes_data.py

    if DEBUG_MODE:
        N_CONTINUOUS_TRANING_EPOCH = 1
        N_REGULAR_EVALUATION_EPOCH = 1
        config_data['training_data']['number_of_data_shards'] = 4
    ...
    if TOGGLE['PART1']:
        ...
        tr.training_vgg_ten_classes(config_data=config_data, realtime_update=realtime_update)
        end = time.time()
        elapsed = end - start
    ...
    if TOGGLE['PART1.2']:   
        print('\nPART1.2')     
        ev.eval_vgg_ten_classes_plot_loss(config_data=config_data, plot_mode='savefig')
    ...
    if TOGGLE['PART2']:
        ...
        for branch_number in BRANCHES_PER_MODEL:
            ...
            ev.eval_vgg_ten_classes_branch_validation_info(config_data=config_data)
            print('\nPART2 branch %s DONE\n\n'%(str(branch_number)))
    ...
    if TOGGLE['PART3']:
        ...
        for branch_number in BRANCHES_PER_MODEL:
            ...
            for xai_mode in XAI_MODES: 
                ...
                ev.eval_vgg_ten_classes_xai(config_data=config_data, FIND_OPTIM_BRANCH_MODEL=True, realtime_update=realtime_update)
                # output is .xai data
            end = time.time()
            elapsed = end - start
