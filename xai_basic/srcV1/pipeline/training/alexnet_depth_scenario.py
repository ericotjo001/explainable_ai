...
from model.adjusted_alexdepth import AdjAlexDepth

from pipeline.workflow_config import DEFAULT_CONFIG_DATA
DEFAULT_CONFIG_DATA['model_name'] = 'depthscenario_0001'
...
DEFAULT_CONFIG_DATA['alex_depth_setting'] = {
    'block_setting':[  
...
def initiate_or_load_model(MODEL_DIR, INFO_DIR, config_data, print_spacing='  '):
    net = AdjAlexDepth(config_data['alex_depth_setting'])
    ...
    if os.path.exists(MODEL_DIR) and os.path.exists(INFO_DIR):
        ...
        evaluator = evaluator.load_pickled_data(pickled_dir=INFO_DIR, tv=(1,0,VERBOSE_THRESHOLD))
    else:
...
def training_alexnet_ten_classes_depth_scenario(config_data=None, realtime_update=True):
    if config_data is None: config_data = DEFAULT_CONFIG_DATA
    print('training_alexnet_ten_classes_depth_scenario(). training scheme:%s'%(str(config_data['training_scheme'])))
    ...
    evaluator.reshape_size = DATA_SIZE_ALEXNET
        ...
        for epoch in range(config_data['n_epoch']):
            ...
            for n_th_shard_iter, k in enumerate(random_shards):
                this_dataset = data10.load_dataset_from_a_shard(k, CACHE_FOLDER_DIR, config_data['data_cache_name'],
                    reshape_size=DATA_SIZE_ALEXNET)
