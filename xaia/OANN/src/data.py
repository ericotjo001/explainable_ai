...
class Data2DInput(object):
    ...
    def show_at_most_three(self,):
        plt.figure(figsize=(14,4))
        for i in [0,1,2]:

            plt.gcf().add_subplot(1,3,i+1)
            im = plt.gca().scatter(self.X[:,0],self.X[:,1], s=5, c= self.Y[:,i], cmap='jet', )
            if not self.X_test is None: 
                im = plt.gca().scatter(self.X_test[:,0],self.X_test[:,1], s=24, c=self.Y_test[:,i], cmap='jet', 
                    marker='x',alpha=0.4)

            plt.colorbar(im)
        plt.title('only showing at most the first 3 dims')
        plt.tight_layout()
        plt.show()
