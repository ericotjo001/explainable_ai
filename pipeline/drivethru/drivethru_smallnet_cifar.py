...
if IS_DEBUG:
	...
	DEBUG_N_ITER_MAX_PER_EPOCH = 100
else:
...
def drivethru_0001_smallnet_cifar(config_data, tab_level=0, verbose=250):
	print('drivethru_0001_smallnet_cifar()')
...
def new_or_load_model(state_tracker, config_data, verbose=0, tab_level=0):
	...
	if config_data['data_from_torch']['cifar']['resize'] is None:
		IMG_SIZE = (32,32)
		net = SmallCNNprobe0001(IMG_SIZE=IMG_SIZE, INPUT_CHANNEL_SIZE =3, relprop_mode=config_data['lrp']['relprop_mode'], verbose=verbose, tab_level=tab_level)
	else:
		IMG_SIZE = config_data['data_from_torch']['cifar']['resize']
		net = SmallCNNprobe0001(set_default_params=False,verbose=verbose, tab_level=tab_level)		
		if IMG_SIZE == (256,256):
