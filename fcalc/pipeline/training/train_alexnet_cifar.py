...
if IS_DEBUG:
	DEBUG_train_alexnet_cifar_LOOP_SIGNAL = 0
	DEBUG_train_alexnet_cifar_LOOP2_SIGNAL = 1
else:
	DEBUG_train_alexnet_cifar_LOOP_SIGNAL = 0
	DEBUG_train_alexnet_cifar_LOOP2_SIGNAL = 0

def train_alexnet_cifar_0001(config_data, tab_level=0, verbose=250):
	print('train_alexnet_cifar_0001()')
	...
	for n_epoch in range(l_epoch, l_epoch + config_data['general']['epoch']):
		...
		for i, data in enumerate(trainloader,0):
			...
			if DEBUG_train_loop_0001(DEBUG_train_alexnet_cifar_LOOP_SIGNAL, net, x, y0, 
			...
			stop_iter, stop_epoch = DEBUG_train_loop_0002(DEBUG_train_alexnet_cifar_LOOP2_SIGNAL, 
...
def setup_state_tracker(config_data, for_training=True, verbose=0, tab_level=0):
	...
	tracker_name = 'alexnet_cifar_0001_'
...
def new_or_load_model(state_tracker, config_data, verbose=0, tab_level=0):
	...
	if config_data['data_from_torch']['cifar']['resize'] is None:
		net = AlexNetLike(INPUT_CHANNEL_SIZE=3, verbose=verbose, tab_level=tab_level)
	else:
		IMG_SIZE = config_data['data_from_torch']['cifar']['resize']
		net = AlexNetLike(INPUT_CHANNEL_SIZE=3, set_default_params=False, 
			relprop_mode=config_data['lrp']['relprop_mode'], verbose=verbose, tab_level=tab_level)
		if IMG_SIZE == (256,256):
