...
def save_pickle(this_dir, this_data):
    output = open(this_dir, 'wb')
    pickle.dump(this_data, output)
    output.close()

def load_pickle(this_dir):
    pkl_file = open(this_dir, 'rb')
    data = pickle.load(pkl_file)
    pkl_file.close()
    return data

class ImageNetDirectQuery(object):
...
class ImageNetClassificationManager(object):
    def __init__(self, INDICES, PROJECT_DIR, split='train', MAIN_DATA_DIR=None):
        ...
        if not os.path.exists(self.PROJECT_DATA_DIR):
            os.mkdir(self.PROJECT_DATA_DIR)
    ...
    def get_subset_by_indices_for_classification(self, n_subset=None, split='train', create_new=True):
        ...
        if not os.path.exists(self.DATASET_DIR) or create_new:
            ...
            self.CURRENT_IMG_LIST, self.CURRENT_LABEL_LIST = load_pickle(self.DATASET_DIR)
            return
        ...
        save_pickle(self.DATASET_DIR, (self.CURRENT_IMG_LIST, self.CURRENT_LABEL_LIST))
...
class ImageNetValidation(ImageNetClassificationManager):
    ...
    def get_val_data_by_index(self, i, as_pytorch_tensor=True):
        ...
        return img, this_index, label_text, img_name
